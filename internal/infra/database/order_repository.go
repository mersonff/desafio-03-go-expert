...
func (r *OrderRepository) List(ctx context.Context, page, limit int) ([]entity.Order, error) {
	offset := (page - 1) * limit
	rows, err := r.Db.QueryContext(ctx, "SELECT id, price, tax, final_price FROM orders LIMIT ? OFFSET ?", limit, offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
